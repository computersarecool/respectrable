...
// It is an example of how you would use the Respectrable API without the getState helper method
// Here all of the the state is sent in invidual messages using the Respectrable API

// TODO: Get IDs for all objects, handle sends as a list, handle the 'call' and 'property' messages
window.addEventListener('load', () => {
  ...
  const masterTrackProps = [
    'name',
    'color'
  ]

  const masterTrackChildren = [
    'mixer_device',
    'devices'
  ]
  ...
  const mixerDeviceChildren = [
    'panning',
    'volume',
    'track_activator'
  ]

  const clipSlotsProps = [
    'has_clip'
  ]
  ...
  const deviceParameterProperties = [
    'name',
    'value'
  ]
  ...
  function getState () {
    getOrSet('live_app', ['call', 'get_major_version'])
    getOrSet('live_app', ['call', 'get_minor_version'])
    getOrSet('live_app', ['call', 'get_bugfix_version'])
    getOrSet('live_set', ['get', 'tracks'])
  }
